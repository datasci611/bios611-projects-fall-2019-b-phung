...
Something of interest would be to see the breakdown of how frequently families visit, so we code that here: We need to count how many times each given client has visited, and appened that to the main data set. However, values of visits ranged from 1 to 249, which is simply too many levels to be useful, so we further group the count by 10s with the last category being 50+.
...
Visit counts are relatively low before 2000 or so, so those proportions are harder to see from the above figure, and may not mean much anyway. By eye, roughly 50 to 60 percent of the visits each year are composed of those coming 10 times or less in the most years in the 2000s, but in the 2010s that proportion appears to have reduced to about 40%. We can confirm by plotting the proportion of visitors who fall into the (0,10] bin to total vistors:
...
knitr::kable(tail(tenure,5), caption = "Shortest continuously visiting clients since 1999 (all one-time vistors)")
